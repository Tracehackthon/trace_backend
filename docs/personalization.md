...
`$trace-adapt` 的适配对话由[对话引擎](dialogue-engine.md)按固定脚本推进：逐题访谈 → 回放确认 → 逐条草案（自动携带历史拒绝理由作为避免方向）→ 可反复修订的协商轮次 → 显式采纳 → 留下 watching 随访。每一轮问答和每个岔路都在项目决策路径中可见、可回放；没有被确认的理解不会进入 profile。
