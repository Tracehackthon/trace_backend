...
const traceUser = window.traceUser || { fullname: 'Trace 用户', headline: '个人工作区' }
const esc = (v = '') => String(v).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#039;')
const icon = (name) => `<i data-lucide="${name}" aria-hidden="true"></i>`
const traceMark = () => '<img class="trace-mark" src="./public/logos/trace.png" alt="" width="40" height="40" />'
...
function sidebar() {
  ...
  return `<aside class="sidebar ${ui.sidebarOpen ? 'sidebar-open' : ''}">
    <a href="#memory" class="brand"><span class="brand-mark">${traceMark()}</span><strong>T R A C E</strong><span class="beta">BETA</span></a>
...
app.addEventListener('click',event => {
  ...
  switch (el.dataset.action) {
    ...
    case 'logout': location.href = './'; break
