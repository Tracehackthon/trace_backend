...
- 产品 `trace-runtime` 当前是 `0.8.0`，Codex bundle 为 `trace.codex@0.5.0`，Codex starter/team/empty template 为 `0.4.0`。它们不表示每个 workspace 包或每个协议都等于相同版本。`trace-codex` Plugin/MCP 已由 Changeset 列入下一次发行计划；在产品 runtime、bundle 与 profile 得到同一次明确发布决策前，不把源码 checkout 误称为用户已安装版本。
