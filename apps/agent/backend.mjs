...
export function createAgentBackend({ webStore, env = process.env, retrievalProvider = null } = {}) {
  ...
  try {
    ...
    return createAgentHttp({ service: createAgentService({ store, readWorkspace: webStore.read, executorRegistry, timeoutMs, retrievalProvider }) });
