...
export function createAgentStore({ file, workspaceKey, maxRuns = 1000 } = {}) {
  ...
  function event(run, type, data) {
    ...
    const value = { protocolVersion: 1, runId: run.id, sequence, type, at: new Date().toISOString(),
      matterId: run.request.matterId, contextEpoch: run.request.contextEpoch, contextHash: run.context.contextHash, data };
  ...
  return {
    ...
    create(request, context) {
      return transaction(() => {
        ...
        const run = { id: randomUUID(), request, requestHash: hash(request), context, status: 'queued', createdAt: new Date().toISOString(),
