...
讨论由[对话引擎](dialogue-engine.md)驱动：`$trace-review` 会逐个候选呈现岔路（保存摘要 / 脱敏片段 / 完整私有 / 拒绝 / 暂不决定），你的每个决定和理由进入项目的决策路径（`trace path` 回放）。拒绝必须给出理由——它会成为以后 `$trace-adapt` 起草协作模型时的负例。CLI 的 `trace review` 路径仍然可用，作为恢复与自动化入口。
