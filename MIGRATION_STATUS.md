...
| Codex adapter | `apps/codex` | 已迁移并验证（真实 native-host hook 路由） | 用户级 `hooks.json` 调用 `hook-stdio --route-from-event-cwd`；受管七个事件为 `SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PostToolUse`、`Stop`、`Interrupt`、`SessionEnd`；cwd 仅发现 `.trace/` 候选，descriptor/Git/worktree 核验通过才绑定，冲突或漂移 fail closed；已附着但无 `.trace/` 的 lifecycle 可进入用户级 Product 数据库，未附着仍不捕获 prompt/final |
...
| Codex hooks 切换 | `packages/host/codex-hooks` + `apps/cli` | 已迁移并验证 | 读写真实 `hooks.json` 形状，管理 SessionStart/UserPromptSubmit/PreToolUse/PostToolUse/Stop/Interrupt/SessionEnd，移除旧 Python / 固定项目 Trace command、保留无关 hooks、备份、CAS preview、rollback；默认不静默修改用户配置 |
...
| 产品 CLI 与文档导航 | `apps/cli` + README.md + docs/ | 已迁移并验证 | 默认帮助只暴露 init/status/upgrade/inbox/review/sources/profile/abilities/codex/doctor/backup；`trace upgrade` 只读显示 runtime/项目 lock 差异，自动变更固定为空；协议参数收进 `trace internal` / --help --advanced；用户文档不依赖个人路径，所有项目状态从当前项目 .trace/ 发现 |
| Codex Plugin、Skills 与本地 MCP | `plugins/trace-codex` + `apps/mcp` + `packages/product/application` | 已实现并验证（Codex-first 对话入口） | `$trace` / `$trace-adapt` / `$trace-review` 以自然语言驱动 status、初始化、旧项目 profile migration、profile update 与 hooks proposal；标准 stdio MCP 不 shell-out 到 CLI，直接复用 instance/runtime/host installer；每次 apply 都要求 proposal fingerprint + `adopt:<proposal_id>`，测试覆盖新项目、来源 root 隐藏、只读 upgrade 与 legacy SQLite-preserving migration；发行包物理化 MCP 依赖，native installer 可完整安装。一次性 Plugin 安装器需要真实 Codex CLI/用户 marketplace 状态，默认 dry-run、显式 confirm/replace，运行时更新不会自动执行它 |
