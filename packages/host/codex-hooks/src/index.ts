...
function noSymlink(target: string): void { let cursor = target; while (true) { let stat: fs.Stats | undefined; try { stat = fs.lstatSync(cursor); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; } if (stat !== undefined) { if (stat.isSymbolicLink()) throw new Error(`Symlink is not allowed: ${target}`); return; } const parent = path.dirname(cursor); if (parent === cursor) return; cursor = parent; } } // Host-level symlinked ancestors above the deepest existing entry (e.g. macOS /var → /private/var) are environment, not managed state.
