...
export function activateCodexTurn(runtime: TraceRuntime, event: CodexTurnStartedEvent): CodexActivationWithReceipt {
  ...
  runtime.createThread({thread_id: threadId, title: `Codex: ${event.purpose}`, current_summary: event.summary, next_action: 'Review the activation notice before reading sources'});
  ...
  const receipt = runtime.createReceipt(buildCodexActivationReceipt({thread_id: threadId, activated_refs: activatedRefs, next_prompts: ['显示本次实际激活的来源和读取指针', '讨论结束后决定是否形成候选沉淀']}));
  return {pack: activation.pack, user_notice: activation.user_notice, receipt};
}
...
export function buildCodexHookOutput(input: CodexHookInput, runtime: TraceRuntime, sourceProfile?: MyWikiSourceProfile): Record<string, unknown> {
  ...
  const visible = {trace: 'activation', source_profile: sourceProfile?.source_id ?? null, pages_considered: pages.map(page => ({path: page.relative_path, title: page.title, revision: page.revision, content_hash: page.content_hash})), pack_id: activation.pack.pack_id, user_notice: activation.user_notice};
