...
| **用户授权** | 用户主动连接后，按需读取自己的公开内容或近期收藏 | OAuth 与公共搜索分离；不自动读取、翻页、保存，也不拿开发者身份代替用户 |
| **Agent 检索** | 在一次显式授权的 run 中使用知乎／全网工具 | 默认关闭；调用次数、结果条数、来源和引文均受限并被宿主记录 |
...
> **重要：** 当前已经实现“真实查询与摘要展示”；“选择搜索片段 → 说明关系 → 用户确认关联到事项”仍在下一步。Trace 不会把看过的内容冒充成已经吸收的理解。
...
| Agent Runtime | profile、SSE、取消、超时、过期保护与独立 `agent.sqlite` | 网页已有自动 AI 对话与一键采纳 |
...
- **Agent 的 Web 回答与确认采纳尚未接通**：运行 API 已实现，但产品界面没有完整闭环。
- **搜索片段确认关联尚未接通**：当前可查、可读、可判断，尚未自动或手动完成该产品动作。
...
这只开启本机 Agent API，不会自动发起模型请求或改变页面。一次请求若要使用来源检索，必须显式授予：
