...
const INSTRUCTIONS = `You are Trace's bounded thinking assistant, not a coding agent for the user's machine.
...
Help discuss, explain, compare supplied evidence, or propose a revision of the exact selected passage. Do not claim web search, file inspection, execution, or real-world verification.
The context manifest contains identities, not fragment bodies. Read relevant fragments using trace_context_read or trace_context_search before reasoning about or quoting them. Only these tools may provide context. Do not use native skills, ask approval, access files, run commands, or access other conversations.
...
export function createCodexAdapter({ executable = process.env.TRACE_CODEX_BIN || 'codex', model = process.env.TRACE_CODEX_MODEL,
  ...
  async function open(signal) {
    ...
    const childEnv = { ...env };
    for (const key of Object.keys(childEnv)) if (/^CODEX_(?:THREAD|SESSION|TURN|TASK|INTERNAL|CALLER|MANAGED)/.test(key)) delete childEnv[key];
  ...
  return {
    ...
    async execute({ request, context, signal, onEvent, isCurrent }) {
      const host = await open(signal), rpc = host.connection;
      let threadId, turnId, completed = false, toolCalls = 0, outputBytes = 0;
      const texts = new Map(), providedContextIds = new Set(), providedFragments = []; let finish, fail;
      ...
      rpc.onRequest = async (method, params) => {
        ...
        demand(++toolCalls <= 12, 'TOOL_BUDGET', '已到达本次工具调用预算。', 429);
        let result, success = true;
        try { result = callContextTool(context, params.tool, params.arguments); }
        catch (error) { success = false; result = { error: error instanceof AgentError ? error.code : 'TOOL_FAILED' }; }
        const contextIds = !success ? [] : params.tool === 'trace_context_read' ? [result.id] : result.matches.map(f => f.id);
        for (const id of contextIds) providedContextIds.add(id);
        if (success) providedFragments.push(...(params.tool === 'trace_context_read' ? [{ id: result.id, text: result.text }]
          : result.matches.map(f => ({ id: f.id, text: f.excerpt }))));
        onEvent('tool.completed', { tool: CONTEXT_TOOLS.some(t => t.name === params.tool) ? params.tool : 'unavailable', success, contextIds });
      ...
      rpc.onNotification = (method, params) => {
        ...
        if (method === 'turn/completed') {
          completed = true;
          if (params.turn.status === 'completed') finish({ raw: [...texts.values()].at(-1) ?? '', threadId, turnId: params.turn.id, runtimeVersion: host.version, providedContextIds: [...providedContextIds], providedFragments });
      ...
      try {
        ...
        const started = await rpc.rpc('thread/start', { cwd: host.cwd, ephemeral: true, approvalPolicy: 'never', sandbox: 'read-only',
          ...
          selectedCapabilityRoots: [], environments: [], dynamicTools: CONTEXT_TOOLS });
        threadId = started.thread.id;
        onEvent('runtime.connected', { threadId, runtimeVersion: host.version, model: started.model });
        current();
        const manifest = { ...context, fragments: context.fragments.map(({ text, ...f }) => ({ ...f, characters: text.length })) };
