...
test('Skill replacement and Codex hooks switch are atomic, explicit and reversible', () => {
  ...
  const hooksFile = path.join(dir, 'hooks.json'); fs.writeFileSync(hooksFile, JSON.stringify({hooks: {UserPromptSubmit: [{hooks: [{type: 'command', command: 'python trace_hook.py'}]}], SessionStart: [], Stop: [{hooks: [{type: 'command', command: 'keep-me'}]}]}})); const hooks = new CodexHookInstaller(hooksFile); const hookReceipt = hooks.install({command: 'node trace-runtime codex hook-stdio --sqlite-state-file C:/state.sqlite', backup_root: path.join(dir, 'hook-backups'), approval: 'approve:codex-hooks'}); const installed = JSON.parse(fs.readFileSync(hooksFile, 'utf8')); assert.equal(installed.hooks.UserPromptSubmit[0].hooks[0].command.startsWith('node trace-runtime'), true); assert.equal(installed.hooks.PreToolUse[0].matcher, '*'); assert.equal(installed.hooks.PostToolUse[0].matcher, '*'); assert.equal(installed.hooks.Stop[0].hooks[0].command, 'keep-me'); hooks.rollback(hookReceipt); assert.equal(JSON.parse(fs.readFileSync(hooksFile, 'utf8')).hooks.UserPromptSubmit[0].hooks[0].command, 'python trace_hook.py');
});

test('A v0.1 hook-install receipt remains rollback-compatible after the v0.2 event expansion', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-hook-legacy-receipt-'));
  const hooksFile = path.join(dir, 'hooks.json');
  const backupRoot = path.join(dir, 'backups'); fs.mkdirSync(backupRoot, {recursive: true});
  const original = '{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"keep-me"}]}]}}\n';
  const current = '{"hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"trace old"}]}]}}\n';
