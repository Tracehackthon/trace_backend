...
export function validateRequest(value) {
  demand(keys(value, ['protocolVersion', 'requestId', 'expectedRevision', 'matterId', 'contextMode', 'contextEpoch', 'purpose', 'input', 'selection', 'sourceIds', 'previousRunId']), 'INVALID_REQUEST', '请求包含不支持的字段。', 400);
...
export function validateOutput(raw, context, request) {
  let value;
  try { value = JSON.parse(raw); } catch { throw new AgentError('INVALID_OUTPUT', 'Codex 没有返回有效的结构化结果。', 502); }
