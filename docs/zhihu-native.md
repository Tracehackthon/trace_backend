...
本页对应工作区源码。接口与受控测试已实现；2026-09-15 已用实际 Access Secret 完成知乎搜索及本机 Web 真实来源展示。全网接口返回正常，但本轮未取得非知乎来源。搜索结果现可由用户选入同一事项的对照，并保留 provider、作者、摘要和 URL。**这不等于线上域名已部署，也不等于真实知乎 OAuth 账号联调通过**。独立 `trace-portal` 浏览器站不自动获得这些本机能力。

## 现在能做什么

| 入口 | 当前能力 | 边界 |
| --- | --- | --- |
| 本机 Web → 个人与设置 → 知乎与全网 | 搜知乎或全网，显示来源摘要、作者和原文链接；发起／检查／断开授权；用户选择后最小读取创作、关注、近期收藏、收藏夹及内容 | 首页查看不写产品；在一件事中可明确选入对照，仍需用户再次确认关系 |
...
- **已完成本机产品闭环**：搜索结果可选入对照并保留来源身份；Trace Agent 回答、联网证据、候选接受／放弃／撤销已有 Web 与 API 验收。
- **仍待线上边界**：独立线上前端接入、多用户授权存储与真实 OAuth 回调联调；本机能力不冒充这些已经完成。
