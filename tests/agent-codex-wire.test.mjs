...
for (const [codeMode, withRetrieval] of [[false, false], [true, false], [false, true], [true, true]]) test(`WIRE (${codeMode ? 'code-mode' : 'direct'}, retrieval=${withRetrieval}): actual Codex payload excludes inherited skills/hooks and limits dynamic tools to this snapshot`,
  { skip: process.env.TRACE_AGENT_WIRE !== '1', timeout: 120000 }, async t => {
    ...
    const provider = new ZhihuProvider({access_secret: 'WIRE_ZHIHU_SECRET_SENTINEL', fetch_impl: async () => Response.json({Code: 0, Data: {Items: [{Title: 'WIRE_SOURCE', ContentText: 'WIRE_SEARCH_EXCERPT', Url: 'https://www.zhihu.com/question/1/answer/2'}]}})});
    const retrieval = createRetrievalSession({provider, sources: withRetrieval ? ['zhihu', 'global'] : []});
    ...
    const server = http.createServer(async (req, res) => {
      ...
      try {
        ...
        assert.ok(!raw.includes('WIRE_ZHIHU_SECRET_SENTINEL'));
        const allowed = new Set(['request_user_input', 'skills', 'trace_context_read', 'trace_context_search', ...(withRetrieval ? ['trace_zhihu_search', 'trace_global_search'] : []), ...(codeMode ? ['exec', 'wait', 'request_user_input_async'] : [])]);
      ...
      const codeItem = { type: 'custom_tool_call', id: 'cm1', call_id: 'code1', name: 'exec', namespace: 'functions',
        input: `text(await tools.trace_context_read({id:"selection"})); ${withRetrieval ? 'text(await tools.trace_zhihu_search({query:"wire query",count:1}));' : ''} text({process:typeof process,fetch:typeof fetch,require:typeof require}); text(ALL_TOOLS.map(t=>t.name)); text(await tools.skills__list({authority:{kind:"orchestrator"}})); text(await tools.skills__read({package:${JSON.stringify(path.join(skill, 'SKILL.md'))}}));` };
      const item = codeMode && index === 1 ? codeItem : !codeMode && index === 1 ? { type: 'function_call', id: 'fc1', call_id: 'call1', name: 'list', namespace: 'skills', arguments: '{"authority":{"kind":"orchestrator"}}' }
        ...
        : !codeMode && withRetrieval && index === 4 ? {type: 'function_call', id: 'fc4', call_id: 'call4', name: 'trace_zhihu_search', arguments: '{"query":"wire query","count":1}'}
        : { type: 'message', id: 'msg1', role: 'assistant', status: 'completed', content: [{ type: 'output_text', text: answer('WIRE_OK', { citations: [{ contextId: 'selection', quote: 'CURRENT_EXPLICIT_FRAGMENT' }, ...(withRetrieval ? [{contextId: retrieval.evidence()[0]?.id ?? 'missing', quote: 'WIRE_SEARCH_EXCERPT'}] : [])] }), annotations: [] }] };
    ...
    const adapter = createCodexAdapter({ runtimeRoot: path.join(root, 'scratch'), env: { ...process.env, CODEX_HOME: home, ZHIHU_ACCESS_SECRET: 'WIRE_ZHIHU_SECRET_SENTINEL' },
    ...
    try { result = await adapter.execute({ request: { purpose: 'explain', input: 'Answer the explicit selection only.' }, context,
      signal: controller.signal, isCurrent: () => true, retrieval, onEvent: (type, data) => events.push({ type, data }) }); }
    finally { clearTimeout(timer); }
    assert.equal(serverError, null, serverError?.stack);
    assert.equal(bodies.length, codeMode ? 2 : withRetrieval ? 5 : 4); assert.match(result.raw, /WIRE_OK/);
    if (withRetrieval) {
      assert.equal(retrieval.evidence().length, 1); assert.ok(result.providedFragments.some(f => f.text === 'WIRE_SEARCH_EXCERPT'));
      assert.ok(events.some(e => e.type === 'tool.completed' && e.data.tool === 'trace_zhihu_search' && e.data.success && e.data.count === 1));
    }
    ...
    if (codeMode) {
      ...
      assert.deepEqual(nested?.sort(), ['clock__curr_time', 'skills__list', 'skills__read', 'trace_context_read', 'trace_context_search', ...(withRetrieval ? ['trace_zhihu_search', 'trace_global_search'] : [])].sort());
